...
class AssignNode(Node):
    def __init__(self, v1, i, v2):
        ...
        self.value = v2
    def evaluate(self):
        if isinstance(self.name.evaluate(), VariableNode):
            self.name = self.name.evaluate()
            d[self.name] = self.value.evaluate()

        elif isinstance(d[self.name.evaluate()], ListNode):
            self.name = self.name.evaluate()
            d[self.name][self.index.evaluate()] = self.value.evaluate()
            print(self.name.evaluate())
            print(d[self.name].evaluate())
...
class IndexNodeL(Node):
    ...
    def evaluate(self):
        if self.v1 in d:
            if self.v2 in d:
                index = d[self.v2]
                return d[self.v1][index]
            else:
                return d[self.v1][self.v2.evaluate()]
        else:
            v1 = self.v1.evaluate()
            v2 = self.v2.evaluate()
            return v1[v2]
...
#                        | variable LBRACKET expression RBRACKET ASSIGN expression SEMICOLON'''
#    if len(p) == 5:
#        p[0] = AssignNode(p[1], None, p[3])
#    else:
#        p[0] = AssignNode(p[1], p[3], p[6])
#    d[p[1].evaluate()] = p[3]
...
#    d[p[1]].evaluate()[p[3].evaluate()] = p[6].evaluate()
...
#conditional statements, including if, ifelse, and while statement
